<template>
  ...
  >
    ...
    >
      ...
      <span class="sr-only">{{ success ? "Success" : "Error" }} icon</span>
    </div>
    ...
    <button
      ...
      @click="$emit('close')"
      :data-dismiss-target="success ? '#toast-success' : '#toast-error'"
      aria-label="Close"
    >
...
const show = computed(() => props.success || props.message);
</script>
...
.toast {
  ...
  animation: slideInRight 0.3s ease-in-out forwards,
    slideOutRight 0.5s ease-in-out forwards 3s;
